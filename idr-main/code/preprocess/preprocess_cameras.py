...
# Gets the fundamental matrix that transforms points from the image of camera 2, to a line in the image of camera 1
...
# Given a point (curx, cury) in image 0, get the maximum and minimum possible depth of the point,
# considering the second image silhouette (index j)
...
# Get all fundamental matrices that transform points from camera 0 to lines in Ps
...
def get_normalization(source_dir, use_linear_init=False):
    ...
    if use_linear_init:
        # Since there is noise in the cameras, some of them will not appear in all the cameras, so we need more points
