...
class ImplicitNetwork(nn.Module):
    def __init__(
            ...
            weight_norm=True,
            multires=0,
            device=torch.device('cpu')  # 添加设备参数
    ):
        super().__init__()
    ...
    def forward(self, input, compute_grad=False):
        ...
        input = input.to(self.device)  # 将输入数据转移到指定设备
    ...
    def gradient(self, x):
        x = x.to(self.device)  # 转移到设备
        x.requires_grad_(True)
        y = self.forward(x)[:,:1]
        d_output = torch.ones_like(y, requires_grad=False, device=self.device)
...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors):
        points, normals, view_dirs, feature_vectors = (
            points.to(self.device), 
            normals.to(self.device), 
            view_dirs.to(self.device), 
            feature_vectors.to(self.device)
        )
...
class IDRNetwork(nn.Module):
    def __init__(self, conf):
        super().__init__()
        self.device = torch.device("mps") if torch.backends.mps.is_available() else torch.device("cpu")  # 自动选择设备
        self.feature_vector_size = conf.get_int('feature_vector_size')
        self.implicit_network = ImplicitNetwork(self.feature_vector_size, **conf.get_config('implicit_network'), device=self.device)
        self.rendering_network = RenderingNetwork(self.feature_vector_size, **conf.get_config('rendering_network'), device=self.device)
    ...
    def forward(self, input):
        ...
        uv = input["uv"].to(self.device)
        pose = input["pose"].to(self.device)
        object_mask = input["object_mask"].reshape(-1).to(self.device)

        ray_dirs, cam_loc = rend_util.get_camera_params(uv, pose, intrinsics, device=self.device)
        ...
        if self.training:
            ...
            eikonal_points = torch.empty(n_eik_points, 3).uniform_(-eik_bounding_box, eik_bounding_box).to(self.device)
            eikonal_pixel_points = points.clone().detach().to(self.device)
